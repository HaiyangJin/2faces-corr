...
# two-sided tests
summary(emm_fie_d[2:3], infer = TRUE, adjust = "none")
```
...
summary(emm_fie_rt[2:3], infer=TRUE, adjust="none", type="response")
```
...
```{r message=FALSE}
# load the package for ANOVA here
library(afex)
```


Condcut repeated-measures ANOVA:
```{r}
...
Condition means in each condition:
```{r}
(emm_iv <- emmeans(aov_iv, ~ Congruency))
```
...
# two-sided tests
summary(emm_fie_iv[2:3], infer=TRUE, adjust="none")
```

```{r}
# one-sided test
summary(emm_fie_iv[2], side=">", infer=TRUE)
```


```{r}
# one-sided test
summary(emm_fie_iv[3], side="<", infer=TRUE)
```
...
#### Version 2 (combine CCF and facilitation & interference)
...
```{r eval=save_figure, include=FALSE}
ggsave(filename = here("figures","fig_pw_acc.pdf"), 
       plot_pw_acc_pub, width = 3.5, height = 3.5*1.2)
```
...
```{r eval=save_figure, include=FALSE}
ggsave(filename = here("figures","fig_pw_rt.pdf"), 
       plot_pw_rt_pub, width = 3.5, height = 3.5*1.2)
```
...
```{r eval=save_figure, include=FALSE}
ggsave(filename = here("figures","fig_scf_acc.pdf"), 
       plot_scf_acc_pub, width = 3.5, height = 3.5*1.2)
```
...
```{r eval=save_figure, include=FALSE}
ggsave(filename = here("figures","fig_scf_rt.pdf"), 
       plot_scf_rt_pub, width = 3.5, height = 3.5*1.2)
```
...
```{r eval=save_figure, include=FALSE}
ggsave(filename = here("figures","fig_ccf_d.pdf"), 
       plot_ccf_d_pub, width = 4.7, height = 3.5*1.2)
```
...
```{r eval=save_figure, include=FALSE}
ggsave(filename = here("figures","fig_ccf_rt.pdf"), 
       plot_ccf_rt_pub, width = 4.7, height = 3.5*1.2)
```

```{r fig.width=11, fig.asp=.7}
fig_hp2 <- 
...
```{r eval=save_figure, include=FALSE}
ggsave(filename = here("figures","fig_hp.pdf"), 
       fig_hp2, width = 11, height = 11*.7)
```

##### Poster figure
```{r fig.width=22, fig.asp=.2}
fig_hp_poster <- 
...
```{r eval=save_figure, include=FALSE}
ggsave(filename = here("figures","fig_hp_poster.pdf"), 
       fig_hp_poster, width = 22, height = 22*.2)
```
...
```{r eval=save_figure, include=FALSE}
ggsave(filename = here("figures","fig_hpe0.pdf"), 
       fig_hpe0, width = 22, height = 22*.2)
```
...
ggsave(filename = here("figures","fig_hpe.pdf"), 
       fig_hpe, width = 10, height = 10*.7)
```
