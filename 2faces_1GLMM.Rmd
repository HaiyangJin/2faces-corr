...
Use `lmm_pw_rt_zcp` as the optimal model.
```{r}
...
if (file.exists(thisfile)) {
  ...
  emm_scf_rt_tmp <- readRDS(thisfile)
  
} else {
  emm_scf_rt_tmp <- emmeans(lmm_scf_rt_opt, ~ Alignment, type="response",
                        lmer.df = "kenward-roger", pbkrtest.limit = 32000)
  saveRDS(emm_scf_rt_tmp, thisfile)
}

summary(emm_scf_rt_tmp)
```
...
summary(cong_emm[2], side=">", infer=TRUE)
```

Interference in d':
```{r}
emm_fie_d <- contrast(emm_fi_probit, interaction="pairwise")
summary(cong_emm[3], side="<", infer=TRUE)
```
...
if (file.exists(thisfile)) {
  ...
  emm_fi_rt_tmp <- readRDS(thisfile)

} else {
  emm_fi_rt_tmp <- emmeans(lmm_fi_rt_opt, ~ Congruency, type = "response",
                        lmer.df = "kenward-roger", pbkrtest.limit = 83000)
  saveRDS(emm_fi_rt_tmp, thisfile)
}

summary(emm_fi_rt_tmp)
```
