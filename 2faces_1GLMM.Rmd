...
plot_pw_acc_pub <- emm_pw_acc %>%
  ...
  # geom_line(linetype="solid", color = dark_colors[1], size = 0.8) +
  geom_errorbar(aes(ymin = asymp.LCL, ymax = asymp.UCL), size=1.5, width=0, 
                alpha = .6, # position = position_dodge(width = 0.1),
                color = dark_colors[1],
                show.legend = F) + 
...
```{r eval=FALSE, include=FALSE}
ggsave(filename = here("figures","individual_hpe","fig_pw_acc.pdf"), 
...
```{r eval=FALSE, include=FALSE}
ggsave(filename = here("figures","individual_hpe","fig_pw_rt.pdf"), 
...
plot_scf_acc_pub <- emm_scf_acc %>%
  ...
  # geom_line(linetype="dashed", color = dark_colors[2], size = 0.8) +
  geom_errorbar(aes(ymin = asymp.LCL, ymax = asymp.UCL), size=1.5, width=0, 
                alpha = .6, # position = position_dodge(width = 0.1),
                color = dark_colors[2],
                show.legend = F) + 
...
```{r eval=FALSE, include=FALSE}
ggsave(filename = here("figures","individual_hpe","fig_scf_acc.pdf"), 
...
```{r eval=FALSE, include=FALSE}
ggsave(filename = here("figures","individual_hpe","fig_scf_rt.pdf"), 
...
plot_ccf_d_pub <- emm_ccf_d %>%
  ...
  ggplot(aes(Alignment, estimate, color=Congruency, group=Congruency)) +
  geom_point(size = 2) + # position = position_dodge(width = 0.1),
  # geom_line(aes(linetype = Congruency), size = 0.8, show.legend = FALSE) +
  scale_color_manual(values=fi_colors) +
  # scale_linetype_manual(values = fi_linetype) +
  geom_errorbar(aes(ymin = asymp.LCL, ymax = asymp.UCL), size=1.5, width=0, 
                alpha = .6, # position = position_dodge(width = 0.1),
...
```{r eval=FALSE, include=FALSE}
ggsave(filename = here("figures","individual_hpe","fig_ccf_d.pdf"), 
       plot_ccf_d_pub, width = 3.5, height = 3.5*1.2)
```
...
```{r eval=FALSE, include=FALSE}
ggsave(filename = here("figures","individual_hpe","fig_ccf_rt.pdf"), 
       plot_ccf_rt_pub, width = 3.5, height = 3.5*1.2)
```
...
```{r eval=FALSE, include=FALSE}
ggsave(filename = here("figures","individual_hpe","fig_hp.pdf"), 
...
```{r eval=FALSE, include=FALSE}
ggsave(filename = here("figures","individual_hpe","fig_hp_poster.pdf"), 
