...
```{r}
rel_pw_acc_w <- cal_re(df_pw_rel_acc, "acc", PW=="whole")
rel_pw_acc_w
```

```{r}
rel_pw_acc_p <- cal_re(df_pw_rel_acc, "acc", PW=="part")
rel_pw_acc_p
```

```{r}
rel_pw_rt_w <- cal_re(df_pw_rel_rt, "rt", PW=="whole")
rel_pw_rt_w
```

```{r}
rel_pw_rt_p <- cal_re(df_pw_rel_rt, "rt", PW=="part")
rel_pw_rt_p
```
...
```{r}
rel_scf_acc_a <- cal_re(df_scf_rel_acc, "acc", Alignment=="aligned")
rel_scf_acc_a
```

```{r}
rel_scf_acc_m <- cal_re(df_scf_rel_acc, "acc", Alignment=="misaligned")
rel_scf_acc_m
```

```{r}
rel_scf_rt_a <- cal_re(df_scf_rel_rt, "rt", Alignment=="aligned")
rel_scf_rt_a
```

```{r}
rel_scf_rt_m <- cal_re(df_scf_rel_rt, "rt", Alignment=="misaligned")
rel_scf_rt_m
```
...
```{r}
rel_ccf_d_ca <- cal_re(df_ccf_rel_d, "d", 
                        Congruency=="congruent", Alignment=="aligned")
rel_ccf_d_ca
```

```{r}
rel_ccf_d_ia <- cal_re(df_ccf_rel_d, "d", 
                        Congruency=="incongruent", Alignment=="aligned")
rel_ccf_d_ia
```

```{r}
rel_ccf_d_cm <- cal_re(df_ccf_rel_d, "d", 
                        Congruency=="congruent", Alignment=="misaligned")
rel_ccf_d_cm
```

```{r}
rel_ccf_d_im <- cal_re(df_ccf_rel_d, "d", 
                        Congruency=="incongruent", Alignment=="misaligned")
rel_ccf_d_im
```

```{r}
rel_ccf_d_iso <- cal_re(df_ccf_rel_d, "d", 
                        Congruency=="isolated", Alignment=="isolated")
rel_ccf_d_iso
```

```{r}
rel_ccf_rt_ca <- cal_re(df_ccf_rel_rt, "rt", 
                        Congruency=="congruent", Alignment=="aligned")
rel_ccf_rt_ca
```

```{r}
rel_ccf_rt_ia <- cal_re(df_ccf_rel_rt, "rt", 
                        Congruency=="incongruent", Alignment=="aligned")
rel_ccf_rt_ia
```

```{r}
rel_ccf_rt_cm <- cal_re(df_ccf_rel_rt, "rt", 
                        Congruency=="congruent", Alignment=="misaligned")
rel_ccf_rt_cm
```

```{r}
rel_ccf_rt_im <- cal_re(df_ccf_rel_rt, "rt", 
                        Congruency=="incongruent", Alignment=="misaligned")
rel_ccf_rt_im
```

```{r}
rel_ccf_rt_iso <- cal_re(df_ccf_rel_rt, "rt", 
                        Congruency=="isolated", Alignment=="isolated")
rel_ccf_rt_iso
```
