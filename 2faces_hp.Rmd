...
```{r}
lm_scfe_acc <- lm(ali ~ mis, scfe_acc)
summary(lm_scfe_acc)
```
...
ccfe_d <- df_ccf_iso %>% 
  ...
  mutate(fac_subt = con_ali - con_mis,
         int_subt = inc_ali - inc_mis,
         fac_subt_iso = con_ali - isolated,
         int_subt_iso = inc_ali - isolated,
         cong_ali_subt = con_ali - inc_ali,
         cong_mis_subt = con_mis - inc_mis,
...
Facilitation
```{r}
lm_ccfe_d_fac <- lm(con_ali ~ con_mis, ccfe_d)
...
```{r}
lm_ccfe_d_fac_iso <- lm(con_ali ~ isolated, ccfe_d)
...
Interference
```{r}
lm_ccfe_d_int <- lm(inc_ali ~ inc_mis, ccfe_d)
...
```{r}
lm_ccfe_d_int_iso <- lm(inc_ali ~ isolated, ccfe_d)
...
```{r}
lm_ccfe_rt_fac <- lm(con_ali ~ con_mis, ccfe_rt)
...
Facilitation (isolated)
```{r}
lm_ccfe_rt_fac_iso <- lm(con_ali ~ isolated, ccfe_rt)
...
Interference (RT)
```{r}
lm_ccfe_rt_int <- lm(inc_ali ~ inc_mis, ccfe_rt)
...
Interference (isolated)
```{r}
lm_ccfe_rt_int_iso <- lm(inc_ali ~ isolated, ccfe_rt)
