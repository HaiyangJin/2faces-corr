...
df_raw <- bind_rows(df_raw_p_tmp, df_raw_t1_tmp) %>% 
  ...
  mutate(Exp_name = as_factor(Exp_name),
         ...
         stim = paste(StudyFace, TestFace, sep="_")) %>% 
  select(-c(Section, StudyFace, TestFace))
...
df_pw <- df_tidy %>% 
  ...
  select(Subject, PW, Feature, Correct, RT) %>% 
...
df_ccf_iso <- df_tidy %>% 
  filter(Task_name == "CCF",
         Section == "main") %>% # only use trials when the top is the same
