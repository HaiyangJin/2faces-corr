...
if (file.exists(thisfile)) {
  ...
  lmm_pw_rt_max <- 
    lmer(log(RT) ~ PW * Feature + (PW * Feature | Subject),
...
if (file.exists(thisfile)) {
  ...
  lmm_scf_acc_max <- glmer(Correct ~ Alignment + (Alignment | Subject),
                           df_scf,
                           family = binomial(link = "logit"),
                           control = glmerControl(optimizer = "optimx", 
                                                  optCtrl = list(method = "nlminb", 
...
if (file.exists(thisfile)) {
  ...
  lmm_scf_rt_max <- lmer(log(RT) ~ Alignment + (Alignment | Subject),
...
if (file.exists(thisfile)) {
  ...
  lmm_ccf_rt_max <- lmer(log(RT) ~ Congruency * Alignment + (Congruency * Alignment | Subject),
...
ccfe_d <- df_ccf_iso %>% 
  ...
  mutate(fac_subt = congruent_aligned - congruent_misaligned,
         ...
         cong_ali_subt = congruent_aligned - incongruent_aligned,
         cong_mis_subt = congruent_misaligned - incongruent_misaligned,
         # ccfe_subt = fac_subt - int_subt, # value is the same as next line
         ccfe_subt = cong_ali_subt - cong_mis_subt)
ccfe_d 
```
...
```{r}
rel_sub1(rel_pw_acc_w, rel_pw_acc_p, pwe_acc$whole, pwe_acc$part)
```
...
```{r}
rel_scf_acc_a <- rel_each(df_scf_rel_acc, "acc", Alignment=="aligned")
rel_scf_acc_a
```

Accuracy for misaligned
```{r}
rel_scf_acc_m <- rel_each(df_scf_rel_acc, "acc", Alignment=="misaligned")
rel_scf_acc_m
```

RT for aligned
```{r}
rel_scf_rt_a <- rel_each(df_scf_rel_rt, "rt", Alignment=="aligned")
rel_scf_rt_a
```

RT for misaligned
```{r}
rel_scf_rt_m <- rel_each(df_scf_rel_rt, "rt", Alignment=="misaligned")
rel_scf_rt_m
```
...
Subtraction of accuracy
```{r}
rel_sub(rel_scf_acc_a, rel_scf_acc_m, scfe_acc$aligned, scfe_acc$misaligned)
```

Regression of accuracy
```{r}
rel_reg(rel_scf_acc_a, rel_scf_acc_m, scfe_acc$aligned, scfe_acc$misaligned)
```

Subtraction of RT
```{r}
rel_sub(rel_scf_rt_a, rel_scf_rt_m, scfe_rt$aligned, scfe_rt$misaligned)
```

Regression of RT
```{r}
rel_reg(rel_scf_rt_a, rel_scf_rt_m, scfe_rt$aligned, scfe_rt$misaligned)
```
...
rel_ccf_d_cong_a <- df_ccf_rel_d %>%
  filter(Alignment == "aligned") %>% # to filter out conditions
  pivot_wider(c(Subject, StimGroup), values_from = d, names_from = Congruency) %>% 
  mutate(d_cong = congruent - incongruent) %>% 
...
rel_ccf_d_cong_m <- df_ccf_rel_d %>%
  filter(Alignment == "misaligned") %>% # to filter out conditions
  pivot_wider(c(Subject, StimGroup), values_from = d, names_from = Congruency) %>% 
  mutate(d_cong = congruent - incongruent) %>% 
  pivot_wider(Subject, values_from = d_cong, names_from = StimGroup) %>% 
  select(-Subject) %>% 
...
rel_ccf_rt_cong_a <- df_ccf_rel_rt %>%
  ...
  mutate(rt_cong = congruent - incongruent) %>% 
...
rel_ccf_d_ca <- rel_each(df_ccf_rel_d, "d", 
                         Congruency=="congruent", Alignment=="aligned")
rel_ccf_d_ca
```
...
rel_ccf_d_ia <- rel_each(df_ccf_rel_d, "d", 
                         Congruency=="incongruent", Alignment=="aligned")
rel_ccf_d_ia
```
...
rel_ccf_d_cm <- rel_each(df_ccf_rel_d, "d", 
                         Congruency=="congruent", Alignment=="misaligned")
rel_ccf_d_cm
```
...
rel_ccf_d_im <- rel_each(df_ccf_rel_d, "d", 
                         Congruency=="incongruent", Alignment=="misaligned")
rel_ccf_d_im
```
...
rel_ccf_d_iso <- rel_each(df_ccf_rel_d, "d", 
                          Congruency=="isolated", Alignment=="isolated")
rel_ccf_d_iso
```
...
```{r}
rel_sub(rel_ccf_d_ca, rel_ccf_d_iso, ccfe_d$congruent_aligned, ccfe_d$isolated)
```

Regression of accuracy (with isolated baselines)
```{r}
rel_reg(rel_ccf_d_ca, rel_ccf_d_iso, ccfe_d$congruent_aligned, ccfe_d$isolated)
```

Subtraction of RT (with isolated baselines)
```{r}
rel_sub(rel_ccf_rt_ca, rel_ccf_rt_iso, ccfe_rt$congruent_aligned, ccfe_rt$isolated)
```

Regression of RT (with isolated baselines)
```{r}
rel_reg(rel_ccf_rt_ca, rel_ccf_rt_iso, ccfe_rt$congruent_aligned, ccfe_rt$isolated)
```

Subtraction of accuracy (with misaligned baselines)
```{r}
rel_sub(rel_ccf_d_ca, rel_ccf_d_cm, ccfe_d$congruent_aligned, ccfe_d$congruent_misaligned)
```

Regression of accuracy (with misaligned baselines)
```{r}
rel_reg(rel_ccf_d_ca, rel_ccf_d_cm, ccfe_d$congruent_aligned, ccfe_d$congruent_misaligned)
```

Subtraction of RT (with misaligned baselines)
```{r}
rel_sub(rel_ccf_rt_ca, rel_ccf_rt_cm, ccfe_rt$congruent_aligned, ccfe_rt$congruent_misaligned)
```

Regression of RT (with misaligned baselines)
```{r}
rel_reg(rel_ccf_rt_ca, rel_ccf_rt_cm, ccfe_rt$congruent_aligned, ccfe_rt$congruent_misaligned)
```
...
Subtraction of accuracy (with isolated baselines)
```{r}
rel_sub(rel_ccf_d_ia, rel_ccf_d_iso, ccfe_d$incongruent_aligned, ccfe_d$isolated)
```

Regression of accuracy (with isolated baselines)
```{r}
rel_reg(rel_ccf_d_ia, rel_ccf_d_iso, ccfe_d$incongruent_aligned, ccfe_d$isolated)
```

Subtraction of RT (with isolated baselines)
```{r}
rel_sub(rel_ccf_rt_ia, rel_ccf_rt_iso, ccfe_rt$incongruent_aligned, ccfe_rt$isolated)
```

Regression of RT (with isolated baselines)
```{r}
rel_reg(rel_ccf_rt_ia, rel_ccf_rt_iso, ccfe_rt$incongruent_aligned, ccfe_rt$isolated)
```

Subtraction of accuracy (with misaligned baselines)
```{r}
rel_sub(rel_ccf_d_ia, rel_ccf_d_im, ccfe_d$incongruent_aligned, ccfe_d$incongruent_misaligned)
```

Regression of accuracy (with misaligned baselines)
```{r}
rel_reg(rel_ccf_d_ia, rel_ccf_d_im, ccfe_d$incongruent_aligned, ccfe_d$incongruent_misaligned)
```

Subtraction of RT (with misaligned baselines)
```{r}
rel_sub(rel_ccf_rt_ia, rel_ccf_rt_im, ccfe_rt$incongruent_aligned, ccfe_rt$incongruent_misaligned)
```

Regression of RT (with misaligned baselines)
```{r}
rel_reg(rel_ccf_rt_ia, rel_ccf_rt_im, ccfe_rt$incongruent_aligned, ccfe_rt$incongruent_misaligned)
```
